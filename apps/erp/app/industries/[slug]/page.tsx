...
export default async function IndustrySolutionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <MarketingPageShell>
      ...
      <section className="industry-related-section"><div><span className="marketing-eyebrow">Explore related operating models</span><h2>Compare how Biloo ERP supports other industries.</h2></div><div>{relatedIndustries.map((item)=><Link href={`/industries/${item.slug}`} key={item.slug}><span>{item.number}</span><strong>{item.shortTitle}</strong><small>{item.eyebrow}</small></Link>)}</div></section>

      <section className="marketing-cta marketing-cta-v2"><div><span>See this workflow using your business context</span><h2>Request a demonstration focused on {industry.shortTitle.toLowerCase()}.</h2><p>Share the team structure, current records and main control problem. Biloo ERP will use the session to show the most relevant workflow.</p></div><div><Link href={`/request-demo?industry=${industry.slug}`} className="marketing-start marketing-large">Request demo</Link><Link href="/industries" className="marketing-demo marketing-large">All industries</Link></div></section>
    </MarketingPageShell>
  );
}
