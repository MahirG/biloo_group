...
export const metadata: Metadata = {
  title: "Compare Biloo ERP",
  description: "Compare Biloo ERP with spreadsheets, notebooks, disconnected tools, desktop software and larger enterprise ERP approaches.",
};
    ...
    <MarketingPageShell>
      <section className="comparison-hub-hero">
        <div>
          ...
          <p>The right system depends on the number of users, control risks, reporting effort, connectivity and specialist requirements. These comparisons explain where Biloo ERP can improve operations and where another approach may still be appropriate.</p>
      ...
      <section className="comparison-disclaimer"><div><span>Comparison policy</span><h2>Biloo ERP does not claim that one ERP is correct for every organization.</h2><p>The pages describe product and operating-model differences based on the current Biloo ERP implementation. Requirements such as global consolidation, complex manufacturing, country-specific statutory reporting or offline operation require separate validation.</p></div><Link href="/migration" className="marketing-demo marketing-large">Review migration planning</Link></section>

      <section className="marketing-cta marketing-cta-v2"><div><span>Bring your current workflow</span><h2>Compare Biloo ERP using the records and controls that matter to your team.</h2><p>A focused evaluation should use your actual transaction flow, reporting needs, user responsibilities and migration constraints.</p></div><div><Link href="/request-demo?topic=comparison" className="marketing-start marketing-large">Request focused demo</Link><Link href="/pricing" className="marketing-demo marketing-large">View ETB pricing</Link></div></section>
    </MarketingPageShell>
  );
}
