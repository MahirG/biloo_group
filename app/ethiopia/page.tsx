...
export const metadata: Metadata = {
  ...
  description: "See how Biloo ERP supports Ethiopian birr, local operating realities, multilingual access, mobile workflows, local onboarding and business management from Addis Ababa.",
};
...
const localAdvantages = [
  ...
  { number: "06", title: "Local implementation support", text: "Work with a Biloo ERP team based in Addis Ababa for product evaluation, setup guidance, migration planning and team onboarding." },
];
    ...
    <MarketingPageShell>
      <section className="local-hero">
        <div>
          ...
          <p>Biloo ERP is developed by Biloo Group in Addis Ababa to help Ethiopian businesses replace fragmented records with one secure, multilingual and decision-ready workspace.</p>
      ...
      <section className="local-reality-section">
        <div><span className="marketing-eyebrow">Move beyond disconnected records</span><h2>Replace common operating friction with one reliable business picture.</h2><p>Biloo ERP is designed for businesses that need practical control today while building a stronger foundation for growth.</p><Link href="/product-tour" className="marketing-dark-link">Explore the product tour →</Link></div>
        <div className="local-reality-table"><header><span>Before Biloo ERP</span><span>With Biloo ERP</span></header>{operatingReality.map(([before,after])=><p key={before}><span>{before}</span><strong>{after}</strong></p>)}</div>
      </section>

      <section className="marketing-section local-readiness-section">
        <div className="marketing-section-heading"><span>Implementation readiness</span><h2>Start from your current reality—not an idealized process.</h2><p>Whether records are in notebooks, spreadsheets or another system, Biloo ERP can help define the setup and migration path before the organization changes its daily process.</p></div>
