...
export const metadata: Metadata = {
  ...
  description: "Learn about Biloo Group, the Addis Ababa team building Biloo ERP for Ethiopian businesses.",
};

const copy = {
  en: {
    eyebrow: "About Biloo Group",
    title: "Building a clearer operating system for Ethiopian business.",
    intro: "Biloo Group is an Addis Ababa product company focused on making daily business records more connected, controlled and useful for decision-making.",
    facts: [
      ["Based in", "Addis Ababa, Ethiopia"],
      ["Flagship product", "Biloo ERP"],
    ...
    missionText: "Biloo ERP is being built around one practical idea: sales, purchasing, inventory, finance, customers, suppliers and reporting should contribute to the same trusted business picture.",
    ...
    founderText: "Mahir leads Biloo ERP product direction, business workflow design and the development of Biloo ERP as a practical platform for Ethiopian organizations.",
    standardsEyebrow: "Our standard",
    standardsTitle: "What customers should expect from Biloo ERP.",
    standards: [
    ...
    contactEyebrow: "Talk to Biloo ERP",
    ...
    demo: "Request a demo",
    email: "Email Biloo ERP",
    resources: "Read business guides",
  },
  am: {
    eyebrow: "ስለ Biloo Group",
    title: "ለኢትዮጵያ ንግድ የበለጠ ግልጽ የሥራ ስርዓት እየገነባን ነው።",
    intro: "Biloo Group በአዲስ አበባ የሚገኝ የምርት ኩባንያ ሲሆን ዕለታዊ የንግድ መዝገቦችን የተገናኙ፣ ቁጥጥር ያላቸው እና ለውሳኔ ጠቃሚ ለማድረግ ያተኩራል።",
    facts: [
    ...
    missionText: "Biloo ERP በአንድ ተግባራዊ ሀሳብ ላይ እየተገነባ ነው፤ ሽያጭ፣ ግዢ፣ ክምችት፣ ፋይናንስ፣ ደንበኞች፣ አቅራቢዎች እና ሪፖርት ለአንድ የታመነ የንግድ ምስል መረጃ ማቅረብ አለባቸው።",
    ...
    founderText: "ማሂር የBiloo ERP የምርት አቅጣጫን፣ የንግድ የሥራ ሂደት ንድፍን እና Biloo ERPን ለኢትዮጵያ ድርጅቶች ተግባራዊ መድረክ አድርጎ የማሳደግ ሥራን ይመራል።",
    standardsEyebrow: "የሥራ ደረጃችን",
    standardsTitle: "ደንበኞች ከBiloo ERP ምን መጠበቅ አለባቸው።",
    standards: [
    ...
    contactEyebrow: "ከBiloo ERP ጋር ይነጋገሩ",
  ...
  return (
    <MarketingPageShell>
      <section className="about-hero">
        <div><span className="marketing-eyebrow">{c.eyebrow}</span><h1>{c.title}</h1><p>{c.intro}</p><div className="marketing-hero-actions"><Link href="/request-demo?source=about" className="marketing-start marketing-large">{c.demo}</Link><a href="mailto:info@erp.biloogroup.com" className="marketing-demo marketing-large">{c.email}</a></div></div>
        <div className="about-fact-grid">{c.facts.map(([label, value]) => <article key={label}><span>{label}</span><strong>{value}</strong></article>)}</div>
      </section>

      <section className="about-mission-section"><div><span>{c.missionEyebrow}</span><h2>{c.missionTitle}</h2><p>{c.missionText}</p></div><div className="about-mission-mark"><img src="/biloo-erp-mark.svg" alt="" width="150" height="150" className="hisab-logo"/><strong>Biloo ERP</strong><span>Business operating system</span></div></section>
      ...
      <section className="about-leadership-section"><div><span className="marketing-eyebrow">{c.leadershipEyebrow}</span><h2>{c.leadershipTitle}</h2><p>{c.founderText}</p></div><article><span className="about-founder-avatar">MA</span><div><strong>{c.founderName}</strong><small>{c.founderRole}</small><a href="mailto:info@erp.biloogroup.com">info@erp.biloogroup.com</a></div></article></section>
